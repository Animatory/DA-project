...
class NormalizeCustom(ImageOnlyTransform):
    def __init__(self, always_apply=False, p=1):
        super(NormalizeCustom, self).__init__(always_apply, p)

    def apply(self, img, **params):
        img = img - img.mean(axis=(0, 1, 2), keepdims=True)
        img = img / img.std(axis=(0, 1, 2), keepdims=True)
        return img.astype('float32')


class ImageAugmentation(ImageOnlyTransform):
    def __init__(self, hflip, xlat_range, affine_std, rot_std=0.0,
                 ...
                 blur_range=None, always_apply=False, p=1):
        super(ImageAugmentation, self).__init__(always_apply, p)
        self.hflip = hflip
    ...
    def apply(self, image: np.array, **params):
        ...
        xf = identity_xf(len(image))

        if self.hflip:
            x_hflip = np.random.binomial(1, 0.5, size=(len(image),)) * 2 - 1
            xf[:, 0, 0] = x_hflip

        if self.scale_x_range is not None and self.scale_x_range[0] is not None:
            xf[:, 0, 0] *= np.random.uniform(low=self.scale_x_range[0], high=self.scale_x_range[1], size=(len(image),))
        if self.scale_y_range is not None and self.scale_y_range[0] is not None:
            xf[:, 1, 1] *= np.random.uniform(low=self.scale_y_range[0], high=self.scale_y_range[1], size=(len(image),))
        if self.scale_u_range is not None and self.scale_u_range[0] is not None:
            scale_u = np.random.uniform(low=self.scale_u_range[0], high=self.scale_u_range[1], size=(len(image),))
            xf[:, 0, 0] *= scale_u
            xf[:, 1, 1] *= scale_u

        if self.affine_std > 0.0:
            xf[:, :, :2] += np.random.normal(scale=self.affine_std, size=(len(image), 2, 2))

        if self.rot_std > 0.0:
            thetas = np.random.normal(scale=self.rot_std, size=(len(image),))
            rot_xf = rotation_matrices(thetas)
            xf = cat_nx2x3(xf, rot_xf)

        if self.xlat_range > 0.0:
            xf[:, :, 2:] += np.random.uniform(low=-self.xlat_range, high=self.xlat_range, size=(len(image), 2, 1))
        ...
        if self.intens_scale_range_lower is not None:
            col_factor = np.random.uniform(low=self.intens_scale_range_lower, high=self.intens_scale_range_upper,
                                           size=(len(image), 1, 1, 1))

            image = (image * col_factor).astype(np.float32)
        ...
        if self.intens_offset_range_lower is not None:
            ...
            image = (image + col_offset).astype(np.float32)

        xf_centred = centre_xf(xf, image.shape[2:])
        for i in range(len(image)):
            if image.shape[1] == 1:
                image[i, 0, :, :] = cv2.warpAffine(image[i, 0, :, :], xf_centred[i, :, :], (image.shape[3], image.shape[2]))
            else:
                image[i, :, :, :] = cv2.warpAffine(image[i, :, :, :].transpose(1, 2, 0), xf_centred[i, :, :],
                                                   (image.shape[3], image.shape[2])).transpose(2, 0, 1)

        if self.blur_range is not None and self.blur_range[0] is not None:
            sigmas = np.random.uniform(low=self.blur_range[0], high=self.blur_range[1], size=(len(image),))
            ...
            for i in range(len(image)):
                ...
                if image.shape[1] == 1:
                    image[i, 0, :, :] = cv2.GaussianBlur(image[i, 0, :, :], (ksize, ksize), sigmaX=sigma)
                else:
                    image[i, :, :, :] = cv2.GaussianBlur(image[i, :, :, :].transpose(1, 2, 0), (ksize, ksize),
                                                         sigmaX=sigma).transpose(2, 0, 1)

        if self.gaussian_noise_std > 0.0:
            image += np.random.normal(scale=self.gaussian_noise_std, size=image.shape).astype(np.float32)
